...
rfecv_model <- rfe(Group ~ ., 
                   data = train_data, 
                   sizes = c(1:30),  # Try different number of features
...
# Select the first 15 features from the selected features list (if there are at least 15)
top_20_features <- selected_features[1:15]


# Subset the original data frame, including 'ANID', 'Group', and the top 15 features
subset_data <- df[, c('ANID', 'Group', top_20_features)]

# View the resulting data frame
View(subset_data)

# Optionally, if you want to save it as a CSV
write.csv(subset_data, "subset_data_with_selected_features_top20.csv", row.names = FALSE)
