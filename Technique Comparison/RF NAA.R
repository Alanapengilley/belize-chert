...
print(summary_df_naa)

# Identify best model
best_idx_naa <- which.max(sapply(results, function(x) x$f1_weighted))
cat("\nBest configuration (by weighted F1):\n")
print(summary_df_naa[best_idx_naa, ])
best_result_naa <- results[[best_idx_naa]]

#----------------------------------------------------------
#  Confusion Matrix for best model
#----------------------------------------------------------
cm_naa <- best_result_naa$confusion
cm_df_naa <- as.data.frame(cm_naa$table)
colnames(cm_df_naa) <- c("Predicted", "Actual", "Freq")

ggplot(cm_df_naa, aes(x = Actual, y = Predicted, fill = Freq)) +
...
#----------------------------------------------------------
top_vars_naa <- best_result_naa$var_imp %>%
  arrange(desc(Overall)) %>%
  slice(1:20)

ggplot(top_vars_naa, aes(x = reorder(Variable, Overall), y = Overall)) +
